...
function getTierInfo(score: number) {
  if (score >= 2600) return { tier: 'renowned', emoji: '👑', flex: 'RENOWNED' };
  if (score >= 2400) return { tier: 'revered', emoji: '👑', flex: 'REVERED' };
  if (score >= 2200) return { tier: 'distinguished', emoji: '👑', flex: 'DISTINGUISHED' };
  if (score >= 2000) return { tier: 'exemplary', emoji: '👑', flex: 'EXEMPLARY' };
  if (score >= 1800) return { tier: 'reputable', emoji: '🏆', flex: 'REPUTABLE' };
  if (score >= 1600) return { tier: 'established', emoji: '⭐', flex: 'ESTABLISHED' };
  if (score >= 1400) return { tier: 'known', emoji: '🔥', flex: 'KNOWN' };
  if (score >= 1200) return { tier: 'neutral', emoji: '⚡', flex: 'NEUTRAL' };
