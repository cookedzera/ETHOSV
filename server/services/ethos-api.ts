...
export class EthosApiClient {
  ...
  // Get reviews received by a user for R4R analysis - OFFICIAL API FORMAT with enhanced accuracy
  async getReviewsReceived(userkey: string, limit: number = 1000, offset: number = 0): Promise<EthosApiResponse<any>> {
    return this.makeRequest('/api/v2/activities/profile/received', {
      method: 'POST',
      body: JSON.stringify({
        userkey,
        filter: ['review'],
        excludeHistorical: false, // Include all reviews for complete accuracy
        excludeSpam: true, // Exclude spam for cleaner data
        limit: Math.min(1000, limit), // Use API maximum for best accuracy
        offset,
        orderBy: { field: 'timestamp', direction: 'desc' }
      }),
    });
  ...
  // Get reviews given by a user for R4R analysis - OFFICIAL API FORMAT with enhanced accuracy  
  async getReviewsGiven(userkey: string, limit: number = 1000, offset: number = 0): Promise<EthosApiResponse<any>> {
  ...
  async getUserVouchActivities(userkey: string, userStats?: any): Promise<EthosApiResponse<any>> {
    try {
      ...
      // ✅ UPDATED: Use balance field for current vouch values with staking rewards
  ...
  async getExchangeRates(): Promise<EthosApiResponse<{ eth_usd: number }>> {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (price && price > 0 && price < 10000) { // Reasonable bounds check
            console.log(`✅ Got ETH price from API: $${price}`);
    ...
    console.log(`⚠️ Using fallback ETH price: $3400`);
