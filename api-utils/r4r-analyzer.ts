...
export class R4RAnalyzer {
  ...
  private async calculateUserR4RScoreOnly(userkey: string): Promise<{
    ...
    try {
      ...
      // Enhanced review fetching with 100% accuracy guarantee
      const getAllReviews = async (getFunction: (userkey: string, limit: number, offset: number) => Promise<any>) => {
        let allReviews: any[] = [];
        let offset = 0;
        const batchSize = 1000; // Use API maximum for best accuracy
        ...
        while (hasMoreData) {
          ...
          const totalAvailable = result.data.total || newReviews.length;
          ...
          // Check if we have all available reviews
          if (allReviews.length >= totalAvailable || newReviews.length < batchSize) {
            hasMoreData = false;
          } else {
            offset += batchSize;
          }
        ...
        return { success: true, data: { values: allReviews, total: allReviews.length } };
      };
  ...
  async analyzeUser(userkey: string): Promise<R4RAnalysis | null> {
    try {
      ...
      // Quick pre-check: if user has very few reviews, return early analysis
      const quickStats = await ethosApi.getEnhancedProfile(userkey);
      if (quickStats.success && quickStats.data?.stats?.review) {
        const reviewStats = quickStats.data.stats.review;
        const totalReceived = (reviewStats.received?.positive || 0) + 
                             (reviewStats.received?.neutral || 0) + 
                             (reviewStats.received?.negative || 0);
        
        // If user has less than 3 reviews, skip detailed analysis for performance
        if (totalReceived < 3) {
          return {
            userkey,
            ...
            totalReviewsGiven: 0,
            reciprocalReviews: 0,
            reciprocalPercentage: 0,
            quickReciprocalCount: 0,
            quickReciprocalPercentage: 0,
            r4rScore: 0,
            riskLevel: 'Low',
            reviewPairs: [],
            allReviews: [],
            networkConnections: [],
            firstReviewDate: new Date().toISOString(),
            ...
            scoreBreakdown: {
              uncappedBaseScore: 0,
              cappedBaseScore: 0,
              baseScoreDescription: 'No significant review activity',
              volumeMultiplier: 1.0,
              volumeDescription: 'Standard volume (minimal activity)',
              accountAgeMultiplier: 1.0,
              accountAgeDescription: 'Standard account age',
              timePenalty: 0,
              timePenaltyDescription: 'No quick reciprocals detected',
              calculationFlow: '0% base rate × 1.0 volume × 1.0 age + 0 time penalty',
              finalCalculation: '0% (Low Risk - Minimal Activity)'
            }
          };
        }
      }

      // Get comprehensive review data using Activities API v2 with complete accuracy
      const [reviewsReceivedResult, reviewsGivenResult] = await Promise.all([
        ethosApi.getReviewsReceived(userkey, 1000), // Use maximum limit for accuracy
        ethosApi.getReviewsGiven(userkey, 1000) // Use maximum limit for accuracy
      ]);
