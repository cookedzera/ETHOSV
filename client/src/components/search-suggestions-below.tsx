...
export function SearchSuggestionsBelow({ 
  ...
  const getTierInfo = (score: number) => {
    ...
    if (score >= 2400) return { tier: 'revered', animation: 'excellent', status: 'revered', color: 'orange' };
    if (score >= 2200) return { tier: 'distinguished', animation: 'excellent', status: 'distinguished', color: 'purple' };
    if (score >= 2000) return { tier: 'exemplary', animation: 'excellent', status: 'exemplary', color: 'emerald' };
    if (score >= 1800) return { tier: 'reputable', animation: 'good', status: 'reputable', color: 'blue' };
    if (score >= 1600) return { tier: 'established', animation: 'good', status: 'established', color: 'red' };
    if (score >= 1400) return { tier: 'known', animation: 'good', status: 'known', color: 'amber' };
    if (score >= 1200) return { tier: 'neutral', animation: 'none', status: 'neutral', color: 'gray' };
    if (score >= 800) return { tier: 'questionable', animation: 'none', status: 'questionable', color: 'orange' };
    return { tier: 'untrusted', animation: 'none', status: 'untrusted', color: 'gray' };
  };
                ...
                <div className="text-right flex-shrink-0 ml-4 relative z-10">
                  <div className="flex items-center gap-2">
                    <div className={`w-3 h-3 rounded-full transition-all duration-300 ${
                      ...
                      color === 'purple' ? 'bg-purple-500 shadow-lg shadow-purple-500/30' :
                      color === 'emerald' ? 'bg-emerald-500 shadow-lg shadow-emerald-500/30' :
                      color === 'blue' ? 'bg-blue-500 shadow-lg shadow-blue-500/30' :
                      color === 'red' ? 'bg-red-500 shadow-lg shadow-red-500/30' :
                      color === 'amber' ? 'bg-amber-500 shadow-lg shadow-amber-500/30' :
                      color === 'gray' ? 'bg-gray-500 shadow-lg shadow-gray-500/30' :
